...
def analyze_trial_halves_density_all_mice(experiment_date, mice_list, trial_range=(1, 16)):
    """
    Analyze first half vs second half hole check density for all mice across trials 1-15
...
def plot_trial_halves_density_by_group(results_df, savefig=False):
    """
    Create separate visualizations for WT and KO groups, showing first half and second half densities for trials 1-15
    """
    ...
    # Plot 1: WT mice - First Half vs Second Half Density
    if len(wt_data) > 0:
        ...
        ax1.errorbar(wt_avg_first['trial'], wt_avg_first['mean'], yerr=wt_avg_first['sem'], 
                    label='First Half', marker='o', color='gray', alpha=0.7, linewidth=2)
        ax1.errorbar(wt_avg_second['trial'], wt_avg_second['mean'], yerr=wt_avg_second['sem'], 
                    label='Second Half', marker='s', color='black', alpha=0.7, linewidth=2)
    
    ax1.set_xlabel('Trial Number')
    ax1.set_ylabel('Hole Check Density (checks/cm)')
    ax1.set_title('WT Mice: First Half vs Second Half Density')
    ax1.legend()
    ax1.set_xlim(0.5, 15.5)
    ax1.set_ylim(0, 0.07)
    ax1.set_xticks(range(1, 16, 2))  # Show every other trial number
    
    # Plot 2: KO mice - First Half vs Second Half Density
    if len(ko_data) > 0:
        ...
        ax2.errorbar(ko_avg_first['trial'], ko_avg_first['mean'], yerr=ko_avg_first['sem'], 
                    label='First Half', marker='o', color='#306ed1', alpha=0.7, linewidth=2)
        ax2.errorbar(ko_avg_second['trial'], ko_avg_second['mean'], yerr=ko_avg_second['sem'], 
                    label='Second Half', marker='s', color='blue', alpha=0.7, linewidth=2)
    
    ax2.set_xlabel('Trial Number')
    ax2.set_ylabel('Hole Check Density (checks/cm)')
    ax2.set_title('KO Mice: First Half vs Second Half Density')
    ax2.legend()
    ax2.set_xlim(0.5, 15.5)
    ax2.set_ylim(0, 0.07)
    ax2.set_xticks(range(1, 16, 2))  # Show every other trial number
    
    plt.suptitle('Hole Check Density: Target A Learning (Trials 1-15)', fontsize=16, weight='bold')
    plt.tight_layout()
    
    if savefig:
        plt.savefig('trial_halves_density_target_A_only.png', dpi=300, bbox_inches='tight')
    
    plt.show()

def statistical_analysis_density(results_df):
    """
    Perform statistical analyses comparing groups and trial halves for hole check density (Trials 1-15)
    """
    
    print("\n" + "="*70)
    print("TRIAL HALVES HOLE CHECK DENSITY STATISTICAL ANALYSIS (Trials 1-15)")
    print("="*70)
    ...
    if len(ko_first_all) > 0 and len(wt_first_all) > 0:
    ...
    if len(ko_second_all) > 0 and len(wt_second_all) > 0:
    ...
    if len(ko_first_all) > 0 and len(ko_second_all) > 0:
    ...
    if len(wt_first_all) > 0 and len(wt_second_all) > 0:
    ...
    # Learning trends for density (trials 1-15 only)
    print(f"\nLearning Trends Analysis (Target A Learning):")
    
    # Early trials (1-3) vs Late trials (13-15)
    early_trials = results_df[results_df['trial'] <= 3]
    late_trials = results_df[results_df['trial'] >= 13]
    
    if len(early_trials) > 0 and len(late_trials) > 0:
        ko_early_first = early_trials[early_trials['group'] == 'KO']['first_half_density']
        ko_late_first = late_trials[late_trials['group'] == 'KO']['first_half_density']
        wt_early_first = early_trials[early_trials['group'] == 'WT']['first_half_density']
        wt_late_first = late_trials[late_trials['group'] == 'WT']['first_half_density']
        
        ko_early_second = early_trials[early_trials['group'] == 'KO']['second_half_density']
        ko_late_second = late_trials[late_trials['group'] == 'KO']['second_half_density']
        wt_early_second = early_trials[early_trials['group'] == 'WT']['second_half_density']
        wt_late_second = late_trials[late_trials['group'] == 'WT']['second_half_density']
        
        if len(ko_early_first) > 0 and len(ko_late_first) > 0:
            print(f"KO First Half Density - Early trials (1-3): {ko_early_first.mean():.4f}, Late trials (13-15): {ko_late_first.mean():.4f}")
            if ko_early_first.mean() > 0:
                print(f"   Change: {((ko_early_first.mean() - ko_late_first.mean()) / ko_early_first.mean() * 100):+.1f}%")
        
        if len(wt_early_first) > 0 and len(wt_late_first) > 0:
            print(f"WT First Half Density - Early trials (1-3): {wt_early_first.mean():.4f}, Late trials (13-15): {wt_late_first.mean():.4f}")
            if wt_early_first.mean() > 0:
                print(f"   Change: {((wt_early_first.mean() - wt_late_first.mean()) / wt_early_first.mean() * 100):+.1f}%")
        
        if len(ko_early_second) > 0 and len(ko_late_second) > 0:
            print(f"KO Second Half Density - Early trials (1-3): {ko_early_second.mean():.4f}, Late trials (13-15): {ko_late_second.mean():.4f}")
            if ko_early_second.mean() > 0:
                print(f"   Change: {((ko_early_second.mean() - ko_late_second.mean()) / ko_early_second.mean() * 100):+.1f}%")
        
        if len(wt_early_second) > 0 and len(wt_late_second) > 0:
            print(f"WT Second Half Density - Early trials (1-3): {wt_early_second.mean():.4f}, Late trials (13-15): {wt_late_second.mean():.4f}")
            if wt_early_second.mean() > 0:
                print(f"   Change: {((wt_early_second.mean() - wt_late_second.mean()) / wt_early_second.mean() * 100):+.1f}%")
    
    # Additional analysis: Distance traveled comparison
    print(f"\nDistance Analysis (Target A Learning):")
...
def main_trial_halves_density_analysis():
    """
    Main function to run the complete trial halves hole check density analysis for trials 1-15 only
    """
    ...
    print(f"Target A training only: Trials 1-15")
    
    # Run analysis (trials 1-15 only)
    results_df, summary_stats = analyze_trial_halves_density_all_mice(
        experiment_date, mice_list, (1, 16))
    ...
    plot_trial_halves_density_by_group(results_df, savefig=True)
